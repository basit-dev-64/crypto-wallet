...
const WalletGenerator = () => {
  ...
  const getBalance = async (wallet: Wallet, pathTypes: String[]) => {
    if (pathTypes[0] === "501") {
      const url =
        "https://solana-mainnet.g.alchemy.com/v2/S_Kvbr0VQuHJZ7rWhDFaBIpF848WQuVJ";
      const data = {
        jsonrpc: "2.0",
        id: 1,
        method: "getBalance",
      ...
      try {
        ...
        const updatedWallets = wallets.map((item) =>
          item.publicKey === wallet.publicKey ? { ...item, balance: response.data.result.value } : item
        );
        setWallets(updatedWallets)
      } catch (error) {
        console.error(error);
        toast.error("Error retriving Balance");
      }
    }
    if (pathTypes[0] === "60") {
      const url =
        "https://eth-mainnet.g.alchemy.com/v2/S_Kvbr0VQuHJZ7rWhDFaBIpF848WQuVJ";
      const data = {
        jsonrpc: "2.0",
        id: 1,
      ...
      try {
        ...
        const etherBalance = BigInt(response.data.result) / BigInt("1000000000000000000");
        console.log(etherBalance)
        const updatedWallets = wallets.map((item) =>
          item.publicKey === wallet.publicKey ? { ...item, balance: etherBalance } : item
        );
        setWallets(updatedWallets)
      } catch (error) {
        console.error(error);
                ...
                <div className="flex flex-col gap-8 px-8 py-4 rounded-2xl bg-secondary/50">
                  ...
                  <div className="flex flex-col w-full gap-2">
                    <div className="flex gap-10 items-center">
                      <Button
                        size={"sm"}
                        onClick={() => getBalance(wallet, pathTypes)}
                      >
                        Get Balance
                      </Button>
                      <p className="text-primary/80 font-medium cursor-pointer hover:text-primary transition-all duration-300 truncate">
